...
@section('logo_design')
    <section id="logo_design" class="bg-heroBg">
        ...
        <div
            ...
            <div class="container flex flex-col mb-32 mt-10 md:mt-0 space-y-12 md:w-2/3">
            ...
            <!-- Image -->
            <div class="container md:w-1/3 mb-10">
                <img class="object-cover mx-auto"
                    src="{{ asset("images/service/service_banner/{$service->service_banner}") }}"
            ...
            @foreach ($service->package as $packages)
                @if ($packages->position == 1)
                    <div class="flex flex-col mx-10 border border-gray-300 shadow-lg md:mx-0 rounded-2xl">
                        <div
                            class="p-6 text-gray-100 bg-gradient-to-r from-purple-600 to-indigo-600 font-space rounded-t-2xl">
                            <h3 class="text-xl text-center uppercase md:text-2xl">Basic package</h3>
                            <p class="text-sm font-bold text-center font-space">
                                @if ($country_code == 'BD')
                                    <span class="text-xl md:text-3xl">BDT</span>
                                @else
                                ...
                                @if ($packages->offer_package_price == null || $packages->offer_price_end_date <= date('Y-m-d'))
                                    @if ($country_code == 'BD')
                                        <span class="text-xl md:text-3xl">{{ $packages->package_price_bdt }}</span>
                                    @else
                                        <span class="text-xl md:text-3xl">{{ $packages->package_price }}</span>
                                    @endif

                                    <span class="text-sm">/start from</span>
                                @elseif (
                                    $packages->offer_package_price != null &&
                                        $packages->offer_price_start_date <= date('Y-m-d') &&
                                        $packages->offer_price_end_date >= date('Y-m-d'))
                        ...
                        @if ($country_code == 'BD')
                            <input type="hidden" name="price" id="price_3" value="{{ $packages->package_price_bdt }}">
                        @else
                ...
                @elseif($packages->position == 2)
                    <div class="flex flex-col mx-10 border border-gray-300 shadow-lg md:mx-0 rounded-2xl">
                        ...
                        @if ($country_code == 'BD')
                            <input type="hidden" name="price" id="price_3"
                                value="{{ $packages->package_price_bdt }}">
                        @else
                            <input type="hidden" name="price" id="price_3" value="{{ $packages->package_price }}">
                        @endif
                ...
                @elseif($packages->position == 3)
                    <div class="mx-10 border border-gray-300 shadow-lg md:mx-0 rounded-2xl">
                        <div
                            class="relative p-6 text-gray-100 bg-gradient-to-r from-redLight to-redDark font-space rounded-t-2xl">
                            <div class="absolute inset-x-0 top-0 flex justify-center -mt-3 ">
                                <div
